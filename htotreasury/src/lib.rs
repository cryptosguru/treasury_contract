...
declare_id!("AHMVst4jghKqSSMDNUWrLjfHQgwD8dod3vFL4KgkJvKZ");
...
pub mod hto_treasury {
    ...
    pub fn create_admin(_ctx: Context<CreatePoolUser>, bump: u8) -> ProgramResult {
    ...
    pub fn harvest(_ctx: Context<Harvest>) -> ProgramResult {
        ...
        let user = _ctx.accounts.user.load_mut()?;
        ...
        let cpi_program = _ctx.accounts.token_program.to_account_info();
        let cpi_ctx = CpiContext::new_with_signer(cpi_program, cpi_accounts, signer);
        token::transfer(cpi_ctx, user.amount)?;
